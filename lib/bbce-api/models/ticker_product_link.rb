...
module BbceApi
  class TickerProductLink
    # Link identification
    attr_accessor :id

    attr_accessor :underlying_asset
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BbceApi::TickerProductLink` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BbceApi::TickerProductLink`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'underlying_asset')
        self.underlying_asset = attributes[:'underlying_asset']
      end
    ...
    def hash
      [id, underlying_asset, features, company_profile, trading_unit, measurement_unit, active, created_at, updated_at].hash
    end
